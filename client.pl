...
#----------------------- Set up status widgit -------------------------------
   $left_frame->Label(text => "Perl Chat Client v$VERSION")
              ->pack(-side => 'top',
                     -anchor => 'nw');
...
#
   if($conn_stat ne 'connected') {
      ...
      $conn = IO::Socket::INET->new(PeerAddr => $host,
                                 PeerPort => $port,
                                 Proto    => 'tcp')
        or popup_err(4);
...
#
#          $rv = $server->send($outbuffer{$server}, 0);
#          unless (defined $rv) {
#             # Whine, but move on.
...
sub popup_about {
   ...
   my $about_text = "Perl Chat version $VERSION\n".
